...
evmc_result execute(VM& vm, const evmc_message& msg, ExecutionState& state, const CodeAnalysis& analysis) noexcept
{
    ...
    const auto result =
        (state.deploy_container.has_value() ?
                evmc::make_result(state.status, gas_left, gas_refund, state.last_opcode_gas_cost, storage_gas_consumed, storage_gas_refund, speculative_cpu_gas_consumed,
    ...
    evmc_revision rev, const evmc_message* msg, const uint8_t* code, size_t code_size, uint64_t evm_version, const evmc_gas_parameters* gas_params) noexcept
{
    ...
    if (vm->validate_eof && eof_enabled && is_eof_container(container) && msg->depth == 0)
    {
        ...
        if (validate_eof(rev, container_kind, container) != EOFValidationError::success)
            return evmc_make_result(EVMC_CONTRACT_VALIDATION_FAILURE, 0, 0, msg->gas_cost, 0, 0, 0, nullptr, 0);
    }
