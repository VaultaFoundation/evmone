...
class InstructionTracer : public Tracer
{
        ...
        int64_t gas, const ExecutionState& state) noexcept override
    {
        ...
        auto gas_cost = instr::gas_costs[state.rev][opcode];
        //HACK
        if(opcode == OP_CREATE || opcode == OP_CREATE2) gas_cost = 32000;
        ...
        m_out << R"(,"refund":)" << std::dec << state.gas_state.cpu_gas_refund();
